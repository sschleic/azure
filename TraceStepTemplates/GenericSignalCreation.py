###################################################################################
# @file      GenericSignalCreation.py
# @brief     TraceStepTemplate to create a generic signal with only one value
#
# @author    FDTech GmbH
# @date      2020-07-02
...
class GenericSignalCreation(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
                            This trace step template takes a reference signal and an input value und emits a new signal with a constant value.
                            ''').strip()
    ...
    def GetSignals(cls):
        ...
        """
        return [
            SignalDefinition('referenceSignal', 'IN', optional=True, description='Reference signal to provide the needed timestamp'),
            SignalDefinition('genericSignal', 'OUT', optional=True, description='New generic signal')
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        for triggerRange in ranges:
            reference = triggerRange.GetValues('referenceSignal')
            timeStamps = triggerRange.GetTimestamps()
            for value in reference:
                outValues.append(threshold)
        signals['genericSignal'].Emit(timeStamps, outValues)
