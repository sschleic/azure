...
class RelativeError(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            Calculates the relative error for each frame.
            ''').strip()
    ...
    def GetSignals(cls):
        ...
        """
        return [
            SignalDefinition('AbsoluteError', 'IN', optional=False, description="Signal representing the absolute error"),
            SignalDefinition('Reference', 'IN', optional=False, description="Signal representing the ground truth."),
            SignalDefinition('RelativeError', 'OUT', optional=False, description="Signal representing resulting relative error for each frame."),
            ]
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition('ReferenceThreshold', 'IN', description="Tolerance level for the reference signal, to account for natural uncertainties. Used to determine if reference signal could be zero.")
        ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # write values of input signals/parameters into local variables, for easier reference
        absoluteErrorValues = signals['AbsoluteError'].GetValues()
        absoluteErrorTimestamps = signals['AbsoluteError'].GetTimestamps()
        referenceValues = signals['Reference'].GetValues()
        referenceValuesTimestamps = signals['Reference'].GetTimestamps()
        tolerance = parameters['ReferenceThreshold']

        # absolute error could only be computed for timestamps at which both sensor and reference signals contained a value
        # consequently, absoluteErrorValues might contain less values than referenceValues
        # consequently, we need to filter referenceValues
        # procedure: find corresponding values in absoluteErrorValues and referenceValues by comparing timestamps
        # third return argument contains indices of common timestamps in reference signal
        validIndicesReference = np.intersect1d(absoluteErrorTimestamps, referenceValuesTimestamps, return_indices=True)[2]
        print(validIndicesReference.size)

        # write valid values referenceValues to new variable
        validReferenceValues = referenceValues[validIndicesReference]

        # create array to store relative errors as well as array to hold indices
        # to access relativeError array later
        relativeError = np.ndarray(len(absoluteErrorValues))
        indices = np.arange(0, len(absoluteErrorValues))
        
        # check if reference value could be zero:
        # if > 0: calculate relative error
        # else: relErr = -1
        relativeErrorTriplet = zip(absoluteErrorValues, validReferenceValues, indices)
        for absError, ref, index in relativeErrorTriplet:
            if abs(ref) - tolerance > 0:
                relativeError[index] = absError / abs(ref)
            else:
                relativeError[index] = -1

        # write relative error to output signal
        signals['RelativeError'].Emit(absoluteErrorTimestamps, relativeError)
