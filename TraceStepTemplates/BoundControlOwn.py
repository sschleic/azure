...
class BoundControlOwn(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        :rtype: unicode
        """
        return u'Compares a signal against an upper and a lower threshold and returns the percentages that lie within and without those thresholds.'
    ...
    def GetParameters(cls):
        ...
        """
        return [ParameterDefinition(u'UpperBound', 'IN', description=u''),
                ParameterDefinition(u'LowerBound', 'IN', description=u''),
                ParameterDefinition(u'PercentageWithinBounds', 'OUT', description=u''),
                ParameterDefinition(u'PercentageOutOfBounds', 'OUT', description=u'')]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        """
        upperBound = parameters[u'UpperBound']
        lowerBound = parameters[u'LowerBound']
        outOfBoundsCounter = 0.0
        totalTime = deque()
        for triggerRange in ranges:
            signal = triggerRange.GetValues(u'Signal')
            time = triggerRange.GetTimestamps()
            # count out-of-bounds signals
            for value in signal :
                if value < lowerBound or value > upperBound :
                    outOfBoundsCounter += 1
            # append timestamps
            totalTime.append(time)
        # concatenate timesteps from trigger ranges
        totalTime = np.concatenate(totalTime)
        # calculate percentage out-of-bounds and percentage within bounds
        percentageOutOfBounds = outOfBoundsCounter / len(totalTime) * 100
        percentageWithinBounds = 100 - percentageOutOfBounds
        # save as parameters
        parameters[u'PercentageWithinBounds'] = percentageWithinBounds
        parameters[u'PercentageOutOfBounds'] = percentageOutOfBounds