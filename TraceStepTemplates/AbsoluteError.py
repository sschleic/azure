...
class AbsoluteError(NumpyBasedTraceStep):
    """
    ...
    def GetSignals(cls):
        ...
        """
        return [
            SignalDefinition('Sensor', 'IN', optional=False, description="Signal representing the data delivered by the sensor."),
            SignalDefinition('Reference', 'IN', optional=False, description="Signal representing the ground truth."),
            SignalDefinition('AbsoluteError', 'OUT', optional=False, description="Signal representing resulting absolute error for each frame."),
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        sensorValues = signals['Sensor'].GetValues()
        sensorValuesTimestamps = signals['Sensor'].GetTimestamps()
        referenceValues = signals['Reference'].GetValues()
        referenceValuesTimestamps = signals['Reference'].GetTimestamps()

        # find timestamps for which both sensor and reference signals contain values
        # first return argument contains common timestamps
        # second return argument contains indices of common timestamps in sensor signal
        # third return argument contains indices of common timestamps in reference signal
        intersectTimestamps, validIndicesSensor, validIndicesReference = np.intersect1d(sensorValuesTimestamps, referenceValuesTimestamps, return_indices=True)

        # write valid values of sensorValues and referenceValues to new variables
        validSensorValues = sensorValues[validIndicesSensor]
        validReferenceValues = referenceValues[validIndicesReference]

        # calculate absolute error with valid values
        absoluteError = abs(validReferenceValues - validSensorValues)

        # write absolute error to output signal
        signals['AbsoluteError'].Emit(intersectTimestamps, absoluteError)
