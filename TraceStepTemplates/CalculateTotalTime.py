...
class CalculateTotalTime(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        :rtype: unicode
        """
        return u'Calculate total time across all trigger ranges.'
    ...
    def GetSignals(cls):
        ...
        return [SignalDefinition(u'Signal', 'IN', optional=False, description=u''),
                SignalDefinition(u'TotalTime', 'OUT', optional=True, description=u'')]
    ...
    def GetParameters(cls):
        ...
        """
        return [ParameterDefinition(u'TotalTime', 'OUT', description=u'')]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        for triggerRange in ranges:
            ...
            if values.size > 0:
                ...
                triggerRange.SetResultText(u'Success.\nTotal time so far: %2.3f' % totalTime)
            else:
                triggerRange.SetResultText(u'No values in the given trigger range.')
                
        # create numpy.ndarray with totalTime as constant value, so as to match length of timestamp array
        valuesArray = numpy.empty((len(numpy.concatenate(resultTimestamps))))
        valuesArray.fill(totalTime)
        print(type(valuesArray))
        print(len(valuesArray))
        
        
        # write new signal
        signals[u'TotalTime'].Emit(
        values=valuesArray,
        timestamps=numpy.concatenate(resultTimestamps))
