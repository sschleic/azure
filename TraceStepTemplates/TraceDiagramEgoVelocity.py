...
class TraceDiagramEgoVelocity(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            This trace step includes a diagram of the Ego velocity in the report.
            ''').strip()
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # generate figure
        figPath = _TraceEgoVelocity(csvList, logoPath)
...
def _TraceEgoVelocity(csvList, logoPath, dpi=300):
    # get csv file path
    egoMotionFile = None
    ...
    velocityKph = egoMotionData['velocity/value'] * 3.6
    # plot data
    fig = plt.figure(dpi=dpi)
    plt.plot(time, velocityKph)
    plt.title('Ego velocity')
    plt.xlabel(r'$t~/~\mathrm{s}$')
    plt.ylabel(r'$v~/~\mathrm{km/h}$')
    plt.grid()
    ...
    figName = 'EgoVelocity_' + egoMotionFile.split('_')[-1].split('.')[0] + '.png'
