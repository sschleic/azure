###################################################################################
# @file      SaveValueGlobalConstant.py
# @brief     TraceStepTemplate to save a given value into a set of global constants
#
# @author    FDTech GmbH
# @date      2020-07-27
...
class SaveValuePyVariable(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            Saves a given value into a global constant. Specifically used for the compare PI. Checks which constant is already used and where to write the own value.
            ''').strip()
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition('CompareValue', 'IN', description='The value which should be writen into the global constant.'),
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # Save the file name for improved readability of the code
        fileName = api.TestEnvironment.ExecutionInfo.GetCurrentPackageFilename()

        # If a new PI is detected it is stored in a fresh dict key
        if fileName not in user.parameterTransfer.ComparePairs.keys():
            # The value itself gets added as a list
            user.parameterTransfer.ComparePairs[fileName] = [compareValue]
        # If the PI is already in the dict the value gets added
        elif fileName in user.parameterTransfer.ComparePairs.keys():
            # Value of the PI gets appended to the list which was created earlier
            user.parameterTransfer.ComparePairs[fileName].append(compareValue)
