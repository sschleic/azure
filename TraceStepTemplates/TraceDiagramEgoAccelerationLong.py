...
class TraceDiagramEgoAccelerationLong(NumpyBasedTraceStep):
    """
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # generate figure
        figPath = _TraceEgoAccelerationLong(csvList, logoPath)
...
def _TraceEgoAccelerationLong(csvList, logoPath, dpi=300):
    # get csv file path
    egoMotionFile = None
    ...
    accelerationLong = egoMotionData['accelerationLong/value']
    # plot data
    fig = plt.figure(dpi=dpi)
    plt.plot(time, accelerationLong)
    plt.title('Ego long. acceleration')
    plt.xlabel(r'$t~/~\mathrm{s}$')
    plt.ylabel(r'$a_\mathrm{long}~/~\mathrm{m/s^2}$')
    plt.grid()
    ...
    figName = 'EgoAccelerationLong_' + egoMotionFile.split('_')[-1].split('.')[0] + '.png'
