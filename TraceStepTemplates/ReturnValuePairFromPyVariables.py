...
# @brief     This tracestep returns a pair of global constant values plus the name of this value pair
#
# @author    FDTech GmbH
# @date      2020-07-27
...
# |------------|---------|-----------------------------------|--------------------|
# | 27.07.2020 | 1.0     | - Initial version                 | Kevin Kleebusch    |
# |------------|---------|-----------------------------------|--------------------|
# |            |         |                                   |                    |
# |------------|---------|-----------------------------------|--------------------|
# |            |         |                                   |                    |
...
class ReturnValuePairFromPyVariables(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            This tracestep returns a pair of global constant values plus the name of this value pair.
            ''').strip()
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition('CompareValueName', 'OUT', description='Name of the parameter for the comparison value pair'),
            ParameterDefinition('CompareValue1', 'OUT', description='Value one of the pair'),
            ParameterDefinition('CompareValue2', 'OUT', description='Value two of the pair'),
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # Save a list of the existing keys
        listOfKeys= list(user.parameterTransfer.ComparePairs.keys())
        # If there are keys in the dict the first set is returned
        if listOfKeys:
            # Since Python 3.7 dicts hold entries in the order the entries were added
            # This means the first entry is the real first entry
            parameters["CompareValueName"] = listOfKeys[0]
            # Currently only 2 parameters are read, but since return values are kind of fixed this is okay and can
            # be enhanced if needed
            parameters["CompareValue1"] = user.parameterTransfer.ComparePairs[listOfKeys[0]][0]
            parameters["CompareValue2"] = user.parameterTransfer.ComparePairs[listOfKeys[0]][1]
            # A entry which was returned can then be removed from the dict
            user.parameterTransfer.ComparePairs.pop(listOfKeys[0])