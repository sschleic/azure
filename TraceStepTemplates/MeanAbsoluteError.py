...
class MeanAbsoluteError(NumpyBasedTraceStep):
    """
    ...
    def GetSignals(cls):
        ...
        """
        return [
            SignalDefinition('AbsoluteErrorSignal', 'IN', optional=False, description='signal that represents the absolute error')
            ]
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition('MeanAbsoluteError', 'OUT', description='Mean absolute error'),
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # write values of input signals into local variables, for easier reference
        absoluteErrorValues = signals['AbsoluteErrorSignal'].GetValues()

        # calculate mean absolute error
        meanAbsoluteError = sum(absoluteErrorValues) / len(absoluteErrorValues)

        # write calculated MAE to return variable
        parameters['MeanAbsoluteError'] = meanAbsoluteError
