...
class TraceDiagramEgoState(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            This trace step includes a diagram of the Ego state in the report.
            ''').strip()
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # generate figure
        figPath = _TraceEgoState(csvList, logoPath)
...
def _TraceEgoState(csvList, logoPath, dpi=300):
    # get csv file path
    egoMotionFile = None
    ...
    state = egoMotionData['egostate']
    # plot data
    fig = plt.figure(dpi=dpi)
    plt.ylim(-0.5, 3.5)
    plt.yticks([0, 1, 2, 3], ['unknown', 'driving', 'stopped', 'crashed'])
    plt.plot(time, state)
    plt.title('Ego state')
    ...
    figName = 'EgoState_' + egoMotionFile.split('_')[-1].split('.')[0] + '.png'
