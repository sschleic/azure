...
class TraceDiagramEgoAccelerationLat(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            This trace step includes a diagram of the Ego lat. acceleration in the report.
            ''').strip()
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        # generate figure
        figPath = _TraceEgoAccelerationLat(csvList, logoPath)
...
def _TraceEgoAccelerationLat(csvList, logoPath, dpi=300):
    # get csv file path
    egoMotionFile = None
    ...
    accelerationLat = egoMotionData['accelerationLat/value']
    # plot data
    fig = plt.figure(dpi=dpi)
    plt.plot(time, accelerationLat)
    plt.title('Ego lat. acceleration')
    plt.xlabel(r'$t~/~\mathrm{s}$')
    plt.ylabel(r'$a_\mathrm{lat}~/~\mathrm{m/s^2}$')
    plt.grid()
    ...
    figName = 'EgoAccelerationLat_' + egoMotionFile.split('_')[-1].split('.')[0] + '.png'
