...
class MeanRelativeError(NumpyBasedTraceStep):
    """
    ...
    def GetDescription(cls):
        ...
        return textwrap.dedent('''
            Calculates the mean relative error.
            ''').strip()
    ...
    def GetSignals(cls):
        ...
        """
        return [
            SignalDefinition('RelativeErrorSignal', 'IN', optional=False, description='signal that represents the relative error')
            ]
    ...
    def GetParameters(cls):
        ...
        """
        return [
            ParameterDefinition('MinPercentValid', 'IN', description="Minimum percentage of relative error values that should be valid (!= -1). If this minimum value is not met, -1 is returned as MeanRelativeError."),
            ParameterDefinition('MeanRelativeError', 'OUT', description="")
            ]
    ...
    def Process(self, parameters, report, timeAxis, ranges, signals):
        ...
        relativeErrorValues = signals['RelativeErrorSignal'].GetValues()
        minPercentValid = parameters['MinPercentValid']

        # select only data points != -1 (which indicates an invalid relative error)
        validIndices = relativeErrorValues != -1
        validRelativeErrorValues = relativeErrorValues[validIndices]

        # check if minimum percentage of relativeErrorValues were valid
        # minimum percentage is given by input parameter MinPercentValid
        # if true, calculate mean realtive error
        # if false, set meanRelativeError to -1
        percentValidValues = validRelativeErrorValues.size / relativeErrorValues.size * 100
        if percentValidValues >= minPercentValid:
            meanRelativeError = sum(validRelativeErrorValues) / validRelativeErrorValues.size
        else:
            meanRelativeError = -1

        # write calculated MRE to return variable
        parameters['MeanRelativeError'] = meanRelativeError
